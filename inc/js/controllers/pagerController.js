(function(){
  'use strict';
    ...
    function PagerController($scope, $rootScope, $route, $location)
	{		
		$scope.pages = [];
		...
		$scope.updateItemsPerPage = function() 
		{
			...
			$location.path('/'+$scope.page+'/0/'+GetLocalStorage(itemsPerPageStorageName, itemsPerPageDefault));
		}
		$scope.updateSelectedPage = function() 
		{
			$location.path('/'+$scope.page+'/'+$scope.selectedPage*$scope.itemsPerPage+'/'+GetLocalStorage(itemsPerPageStorageName, itemsPerPageDefault));
		}
		...
		$rootScope.$on('calculateImagesPaging', function(event, allCount) 
		{ 
